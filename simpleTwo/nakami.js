...
function update() {
    ...
    let spd = player.spd;
    if(OBS.keys.a){
        if(OBS.keys.space && has('dashTabi') && player.dashIng != 1) spd *= 10, player.dashIng = -1;
        player.dx = -spd;
    }
    if(OBS.keys.d){
        if(OBS.keys.space && has('dashTabi') && player.dashIng != -1) spd *= 10, player.dashIng = 1;
    ...
    if(OBS.keys.w && (player.grounded || hasp(player, 'ghost'))){
        player.dy = player.jumP;
        player.grounded = 0;
    }
    if(OBS.keys.q && OBS.keys.e && has('jetPack')){
        player.dy = player.jumP*0.6;
        player.grounded = 0;
    }
    if(OBS.keys.s && !player.grounded) player.dy += 1; // 急降下

    if(!OBS.keys.a && !OBS.keys.d) player.dashIng = 0;
...
document.addEventListener("keydown", async  function(e){
    ...
    if(OBS.keys.shift && has('shadowRing') && !hasp(player, 'ghost')){
        propAdd(player, 'ghost');
        player.img = 'players/ghost';
        player.jumP *= 9/10
        while(OBS.keys.shift) await delay(10);
