...
function fontsLoad(){
    ...
    let css = Fonts.map(f => {
        let src = `url('index/assets/fonts/${f.src}.${f.type}')`;
...
loaF.loadI = async() => {
    ...
    for(let belong in Images){
        ...
        for(let name of Images[belong]){
            let img = new Image();
            img.src = `index/assets/images/${belong}/${name}.png`;
            img.onload = kasan();
            img.onerror = () => {
                console.error(`Image index/assets/images/${belong}/${name}.png failed to load.`);
                loaC.erd += 1;
                 if(loaC.erd > 20) return console.error('さすがにやりすぎbonus'), 1;
                img.src = `index/assets/images/systems/error.png`;
                kasan();
            };
...
loaF.loadS = async() => {
    ...
    for(let belong in Sounds){
        ...
        for(let name of Sounds[belong]){
            let sound = new Audio();
            sound.preload = 'auto';
            sound.src = `index/assets/sounds/${belong}/${name}.mp3`;
            if(belong == 'bgm'){
                sound.loop = 1;
            ...
            sound.onerror = () => {
                console.error(`Sound index/assets/sounds/${belong}/${name} failed to load.`);
                loaC.erd += 1;
                 if(loaC.erd > 20) return console.error('さすがにやりすぎbonus'), 1;
                sound.src = `index/assets/sounds/se/error.mp3`;
                kasan();
            };
...
let secrates = [
    ...
    },
    {
        ...
        func: async function(){
            ...
            img.id = 'hakaisatsu';
            img.src = 'index/assets/images/systems/hakai_1.png'
            img.dataset.phase = 1;
    ...
    },
    {
        ...
        func: async function(){
            ...
            img.src = 'index/assets/images/systems/hakai_2.png'
            img.dataset.phase = 2;
...
frninF.start = function(){
    ...
    frninC.startD.querySelector('img').src = 'index/assets/images/systems/palsy.png';
...
frninF.end = function(){
    frninC.playing = 0;
    frninC.startD.querySelector('img').src = 'index/assets/images/systems/power.png';
...
frninF.throw = function(){
    ...
    div.style.background = `url('index/assets/images/foods/${f.name}.png') no-repeat center/cover`;
...
window.addEventListener('pointermove', (e) => {
    ...
    if(!OBS.cling) return lastX = null;
