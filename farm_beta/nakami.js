...
let dooC = {
    ...
    shoulD:{
        ...
        jump: dooD.querySelector(".area.jump"),
        forage: dooD.querySelector(".area.forage"),
    }
};
let dooF = {};
...
dooC.jumF.update = () => {
    dooC.jumC.kitie = fibo(dooC.jumC.ed+1)*10;
...
dooC.jumF.start = async() => {
    ...
    let bet = dooC.jumC["kitie"]*dooC.jumC["vai"];
...
dooC.jumF.end = async(ed) => {
    ...
    let vai = dooC.jumC.vai;
    let gaku = dooC.jumC.gaku * vai;
    console.log(`　=> ${dooC.jumC.gaku} * ${vai} | Ɍ${gaku}`);
    rimiF.inc(gaku);
